...
def get_args():
    ...
    fuzzopts.add_argument(
        ...
        type=number_parse,
        default=None,
        help="Enable heap address sanitizer with the given heap base"
    )
    ...
    parser.add_argument(
        "--additional-hooks",
        help="Path to a python file containing additional hooks"
    )
...
def main() -> int:
    ...
    if external_peripherals == 1:
        mod.add_peripherals(loader)

    machine = loader.get_machine()
    machine.modkit.append_search_path(f"{path.dirname(__file__)}/modkit/{loader.NAME}/build")
    ...
    if loader.NAME == "shannon":
        ...
        if args.exclusive:
            machine.enable_tasks_exclusive(args.exclusive)
