...
class FirmWireGuestLogger:
    ...
    def log_emit(self, fmt, *args, **meta):
        ...
        if (
            ...
            if self._disabled_streak > 0 and False:
                self._write(
            ...
            if (self._disabled_streak % 1000) == 0 and False:
                self._write(
        ...
        if self.task_log_buffered(task_name):
            if this_msg is None:
                this_msg = (fmt) % (args)
            self.task_log_put(task_name, this_msg)
    ...
    def task_log_del_buffer(self, *tasks):
        for task_name in tasks:
            del self._tasks_log_buffer[task_name]

    def task_log_put(self, task_name, msg):
        self._tasks_log_buffer[task_name].put(msg)
    ...
    def task_log_buffer_clear(self, task_name, msg):
        self._tasks_log_buffer[task_name].queue.clear()
