...
class FirmWireEmu(ABC):
    ...
    def symbol_table(self):
        return getattr(self.loader, "symbol_table")

    @property
    def task_layout(self):
        return getattr(self.loader, "task_layout")
    ...
    def set_breakpoint(self, address, handler, temporary=False, continue_after=False):
        ...
        func_name = handler.func.__name__ if type(handler) is partial else handler.__name__
        if not func_name.startswith("FirmWireEmu."):
            basename = "_bp_firmwire_%s_%s" % (func_name, address)
    ...
    def run_until(self, address: Union[int, List[int]], arguments: Optional[List[tuple]] = None, temporary=False):
        ...
        if isinstance(address, int):
            bid = self.set_breakpoint(address, partial(bp_handler, event, arguments), temporary=temporary, continue_after=False)
        elif isinstance(address, list):
            assert arguments is None, "`arguments` are not supported with multiple execution targets"
            for addr in address:
                self.set_breakpoint(addr, partial(bp_handler, event, None), temporary=temporary, continue_after=False)
        self.qemu.cont()
        event.wait()
        # More wait
    ...
    def add_panda_hook(self, address, hook, cb_type="start_block_exec", **kwargs):
        ...
        # Wrapper to pass `self' and keyword args to panda hooks
        ...
        self.qemu.add_hook(address, hook_wrapper(hook), cb_type=cb_type)

    def install_mem_hooks(self, mappings):
        """Installs and enables user provided mem hooks."""
        for hook in mappings:
            if "write" not in hook:
                hook["write"] = False
