...
memory_map_inv2 = dict([[v, k] for k, v in memory_map2.items()])
...
class SHM2Peripheral(FirmWirePeripheral):
    def hw_read(self, offset, size):
        offset_name = None
        if offset == 0x0:
            value = self.magic
            ...
            value = self.raw_tx_buff.tail
        elif offset == 0x60:  # CMSG offset
            value = 0x8F940000 + 0x80 - self.address  # SIPC[0x80] - base
            offset_name = "sipc"
        elif offset == 0x64:  # SRINFO offset
            value = 0
        ...
        if offset_name is None:
            if offset in memory_map_inv2:
                offset_name = memory_map_inv2[offset]
            else:
                offset_name = "unknown"
    ...
    # def send_fmt_packet(self, pkt):
    #     self.fmt_tx_buff.queue(pkt)
