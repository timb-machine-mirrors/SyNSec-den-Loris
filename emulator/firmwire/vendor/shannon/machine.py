...
class ShannonMachine(FirmWireEmu, ShannonOSI):
    ...
    def is_emu_stopped(self) -> bool:
        return self.qemu.state & TargetStates.STOPPED
    ...
    def pal_waiting_for_msg(self, task_name, qid_name):
        if qid_name == "SAEL3":
            pass
            # self.sael3.waiting_for_msg()

    def pal_log_send_msg(self, task_name, qid_name, msg):
        # if qid_name == "SAEL3":
        #     self.sael3.msg_received(msg)
        ...
        if msg.size > 0:
            body = hexdump(msg.data, group=1, columns=0x20)
    ...
    def get_args(self):
        ...
        stack_args = self.panda.virtual_memory_read(cpu, sp, 4 * 7)
...
r12: %08x     cpsr: %08x""" % (
    ...
    def hook_debug(self, additional_hooks=None):
        ...
        self.install_hooks(shannon.hooks.mappings + new_mappings)
    ...
    def find_safe_rwx_region(self):
        ...
        for m in sorted(self.avatar.memory_ranges, key=lambda x: x.begin, reverse=True):
            ...
            if (
                ...
                # double check that our region won't be overwritten during the boot process
                for start, end in self.unsafe_regions:
                    if start <= m.begin < end or start < m.end < end:
                        good = False
                        break
    ...
    def initialize(self, loader, args):
        ...
        if rwx_region is None:
            ...
            if args.asan:
                self.heap = intervaltree.Interval(args.asan, args.asan + 0x100000)
                self.heap_top = self.heap.begin
        ...
        free = self.symbol_table.lookup("pal_MemFree_Wrapper")
        if args.asan and malloc is not None and free is not None:
            self.install_hooks([
                {
                    "name": "PIC_MemAlloc",
                    "address": malloc.address,
                    "handler": shannon.hooks.fw_alloc,
                }, {
                    "name": "pal_MemFree",
                    "address": free.address,
                    "handler": shannon.hooks.fw_free,
                },
        ...
        if args.fuzz_triage:
            ...
            log.info(
                "Triage mode active. Logging code coverage to %s", coverage_fn.to_path()
            )
        ...
        if self.modem_soc.name == "S5123" or self.modem_soc.name == "S5133AP":
        ...
        self._panda = self.qemu.pypanda
        shannon.hooks.panda = self._panda
        ...
        self._panda.athread.warned = True
    ...
    def remove_task_by_name(self, name):
        log.info(f"Removing task {name}")
        task_idx = self.find_existing_task_slot(name)
        if task_idx == -1:
            log.warning(f"Task {name} not found")
            return
        log.info(f"Found task index {task_idx}")
        num_tasks = len(self.get_task_list())
        task_arr = self.symbol_table.lookup("SYM_TASK_LIST").address
        remaining_tasks = num_tasks - task_idx - 1
        curr_task_addr = task_arr + task_idx * self.task_layout.SIZE()
        next_task_addr = curr_task_addr + self.task_layout.SIZE()
        data = self.qemu.rm(
            next_task_addr, self.task_layout.SIZE() * remaining_tasks, raw=True
        )
        data += bytearray(self.task_layout.SIZE())
        self.qemu.wm(curr_task_addr, len(data), data, raw=True)

    def enable_tasks_exclusive(self, tasks):
        log.info(f"Running with tasks {tasks}")
        task_data = bytearray(0)
        task_arr = self.symbol_table.lookup("SYM_TASK_LIST").address
        task_list = self.get_task_list()
        for task_idx, task in enumerate(task_list):
            if task.name in tasks:
                task_addr = task_arr + task_idx * self.task_layout.SIZE()
                task_data += self.qemu.rm(task_addr, self.task_layout.SIZE(), raw=True)
                tasks.remove(task.name)
        task_data += bytearray(self.task_layout.SIZE())  # blank task
        self.qemu.wm(task_arr, len(task_data), task_data, raw=True)
        if len(tasks) > 0:
            log.warning(f"Tasks not found: {tasks}")

    def update_task_main_func_by_name(self, name, main_func):
        task_idx = self.find_empty_or_existing_task_slot(name)
        task_arr = self.symbol_table.lookup("SYM_TASK_LIST").address
        curr_task_addr = task_arr + task_idx * self.task_layout.SIZE()
        data = self.qemu.rm(
            curr_task_addr, self.task_layout.SIZE(), raw=True
        )
        new_data = data[:self.task_layout.TASK_MAIN_FN_OFFSET] \
            + struct.pack("<I", main_func) \
            + data[self.task_layout.TASK_MAIN_FN_OFFSET + 4:]
        self.qemu.wm(curr_task_addr, len(new_data), new_data, raw=True)
    ...
    def find_existing_task_slot(self, name):
    ...
    def find_empty_or_existing_task_slot(self, name):
        idx = self.find_existing_task_slot(name)
        if idx != -1:
            return idx
    ...
    def handle_soc_quirks(self):
        ...
        # HACK for CP_G950FXXU1AQI7 and G960
            ...
            # "error - Unknown Freq value in hw_ClkFindSysClkCofigInfoIndex()"
            self.set_breakpoint(
                self.symbol_table.lookup("QUIRK_SXXXAP_DVFS_HACK").address, ff2
            )
