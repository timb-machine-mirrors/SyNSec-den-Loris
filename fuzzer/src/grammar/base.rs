...
pub enum ParserExpr<'i> {
    ...
    Seq(Box<ParserNode<'i, ParserExpr<'i>>>, Box<ParserNode<'i, ParserExpr<'i>>>),
    ...
    Choice(Box<ParserNode<'i, ParserExpr<'i>>>, Box<ParserNode<'i, ParserExpr<'i>>>),
...
impl<'i> LorisGrammarParser<'i, AstRule<Expr>, Expr, ParserExpr<'i>> for LorisParser {
    ...
    fn consume_expr(
        ...
        let term = |pair: Pair<'i, Rule>| Self::unaries(pair.into_inner().peekable(), pratt);
        let infix = |lhs: Result<ParserNode<'i, ParserExpr<'i>>, Vec<Error<Rule>>>,
                     op: Pair<'i, Rule>,
                     rhs: Result<ParserNode<'i, ParserExpr<'i>>, Vec<Error<Rule>>>| match op.as_rule() {
...
impl<'i> LorisGrammarValidator<'i, ParserExpr<'i>> for LorisParser {
    ...
    fn is_non_failing(
        ...
        match *expr {
            ...
            ParserExpr::Seq(ref lhs, ref rhs) => {
                Self::is_non_failing(&lhs.expr, rules, trace) && Self::is_non_failing(&rhs.expr, rules, trace)
            }
    ...
    fn is_non_progressing(
        ...
        match *expr {
            ...
            ParserExpr::Seq(ref lhs, ref rhs) => {
                Self::is_non_progressing(&lhs.expr, rules, trace)
                    && Self::is_non_progressing(&rhs.expr, rules, trace)
            }
            ParserExpr::Choice(ref lhs, ref rhs) => {
                Self::is_non_progressing(&lhs.expr, rules, trace)
                    || Self::is_non_progressing(&rhs.expr, rules, trace)
            ...
            let mut errors = rule
                ...
                .filter_map_top_down(|node| match node.expr {
                    ParserExpr::Choice(ref lhs, _) => {
                        ...
                        if Self::is_non_failing(&node.expr, &map, &mut vec![]) {
                            Some(Error::new_from_span(
                                ErrorVariant::CustomError {
                                    message:
    ...
    fn validate_left_recursion(
        ...
        match node.expr.clone() {
            ...
            ParserExpr::Seq(ref lhs, ref rhs) => {
                if Self::is_non_failing(&lhs.expr, rules, &mut vec![trace.last().unwrap().clone()]) {
                    Self::validate_left_recursion(rhs, rules, trace)
                } else {
                    Self::validate_left_recursion(lhs, rules, trace)
                }
            }
            ParserExpr::Choice(ref lhs, ref rhs) => {
                Self::validate_left_recursion(lhs, rules, trace).or_else(|| Self::validate_left_recursion(rhs, rules, trace))
            }
...
impl LorisGrammar for LorisBaseGrammar {
